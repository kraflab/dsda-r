class Demo < ApplicationRecord
  ...
  scope :movies, -> { reorder(:level, :category_id, :tics).where("level LIKE 'Ep%' OR level LIKE '%A%'") }
  scope :ils, -> { reorder(:level, :category_id, :tics).where("level NOT LIKE 'Ep%' AND level NOT LIKE '%A%'") }
